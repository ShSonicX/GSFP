...
class TestOptionPricer(unittest.TestCase):
    ...
    '''
    def test_milevsky_posner(self):
        self.weights = np.ones(4) * 0.25
        self.prices = np.ones(4) * 100.0
        self.vol = 0.4
        self.corr = np.array([[1.0, 0.5, 0.5, 0.5],
                              [0.5, 1.0, 0.5, 0.5],
                              [0.5, 0.5, 1.0, 0.5],
                              [0.5, 0.5, 0.5, 1.0]])
        self.strike = 100.0
        ...
        mp_price = opt.get_milevsky_posner_price()
        self.assertAlmostEqual(mp_price, 28.10, places=2) 
