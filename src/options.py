...
class BasketOption:
    ...
    def get_milevsky_posner_price(self):
        F = self.prices * np.exp(self.rate * self.time)
        M = np.dot(self.weights, F)
        V2 = 0
        for i in range(len(self.weights)):
            for j in range(len(self.weights)):
                V2 += self.weights[i] * self.weights[j] * F[i] * F[j] * np.exp(
                    self.vol * self.vol * self.corr[i][j] * self.time
                )
        alpha = (2 * V2 - M**2) / (V2 - M**2)
        beta = (V2 - M**2) / (V2 * M)
        g1 = gamma.cdf(1 / self.strike, a=alpha - 1, scale=beta)
        g2 = gamma.cdf(1 / self.strike, a=alpha, scale=beta)
        basket_call = np.exp(-self.rate * self.time) * (M * g1 - self.strike * g2)
        return basket_call
